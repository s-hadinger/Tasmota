...
class Tasmota
  ...
  #
  # Args:
  #    - `wd_or_instance`: (string or instance) working dir indicating which .tapp file to read
  #                        or instance containing either 'wd' attributes or 'wd' method
  ...
  def read_extension_manifest(wd_or_instance)
    ...
    try
      import json
      import string
      # convert instance to wd
      if type(wd_or_instance) == 'instance'
        import introspect
        var wd_member = introspect.get(wd_or_instance, 'wd')
        if type(wd_member) == 'function'
          wd = wd_member(wd)              # call 'wd()' method
        else
          wd = wd_member
        end
      end
