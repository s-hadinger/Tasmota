...
const Z_AttributeConverter Z_PostProcess[] PROGMEM = {
  ...
  { Zuint16,  Cx0012, 0x0055,  Z_(MultiInValue),         0 },
...
public:
  ...
  void parseReportAttributes(Z_attribute_list& attr_list, uint8_t offset = 0);
  void generateSyntheticAttributes(Z_attribute_list& attr_list);
  void parseReadAttributes(JsonObject& json, uint8_t offset = 0);
  void parseReadAttributesResponse(Z_attribute_list& attr_list, uint8_t offset = 0);
  void parseReadConfigAttributes(Z_attribute_list& attr_list, uint8_t offset = 0);
  void parseConfigAttributes(Z_attribute_list& attr_list, uint8_t offset = 0);
  void parseResponse(void);
  void parseClusterSpecificCommand(JsonObject& json, uint8_t offset = 0);
  void postProcessAttributes(uint16_t shortaddr, JsonObject& json, Z_attribute_list& attr_list);
...
// OLD CODE JSON
uint32_t parseSingleAttributeOld(JsonObject& json,
                              char *attrid_str, class SBuffer &buf,
                              uint32_t offset, int32_t attrtype = -1) {
...
void ZCLFrame::parseReportAttributes(Z_attribute_list& attr_list, uint8_t offset) {
  uint32_t i = offset;
...
void ZCLFrame::parseConfigAttributes(Z_attribute_list& attr_list, uint8_t offset) {
  uint32_t i = offset;
  uint32_t len = _payload.len();
...
void ZCLFrame::parseReadConfigAttributes(Z_attribute_list& attr_list, uint8_t offset) {
  uint32_t i = offset;
  uint32_t len = _payload.len();
...
void ZCLFrame::parseReadAttributesResponse(Z_attribute_list& attr_list, uint8_t offset) {
  uint32_t i = offset;
  uint32_t len = _payload.len();
...
int32_t Z_OccupancyCallback(uint16_t shortaddr, uint16_t groupaddr, uint16_t cluster, uint8_t endpoint, uint32_t value) {
  ...
  JsonObject& json = jsonBuffer.createObject();
  json[F(OCCUPANCY)] = 0;
  Z_attribute_list attr_list;   // TODO 
  zigbee_devices.jsonPublishNow(shortaddr, json, attr_list);
...
int32_t Z_ApplyConverter(const class ZCLFrame *zcl, uint16_t shortaddr, JsonObject& json, const char *name, JsonVariant& value, const String &new_name,
                        uint16_t cluster, uint16_t attr, int8_t multiplier) {
...
void ZCLFrame::postProcessAttributes(uint16_t shortaddr, JsonObject& json, Z_attribute_list& attr_list) {
  // source endpoint
  uint8_t src_ep = _srcendpoint;
  // TODO review new iterator
  for (auto &attr : attr_list) {
    // attr is Z_attribute&
    if (!attr.key_is_str) {
    ...
    if (delimiter) {
      ...
      for (uint32_t i = 0; i < ARRAY_SIZE(Z_PostProcess); i++) {
        ...
        if ((conv_cluster == cluster) &&
          ...
          int32_t drop = Z_ApplyConverter(this, shortaddr, json, key, value, new_name_str, conv_cluster, conv_attribute, conv_multiplier);
          if (drop) {
            json.remove(key);
          }
