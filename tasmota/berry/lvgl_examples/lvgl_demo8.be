...
btn_style.set_text_color(lv_color(0xFF0000))    # text color white #FFFFFF
...
# def event_cb_dispatch(event_addr)
#     print(">> event_cb_dispatch")
#     import introspect
#     var event = lv_event(introspect.toptr(event_addr))  # create a mapped bytes() to the event structure
#     var target = lv_obj(nil, event.target)      # map to a simple object
#     btn_clicked_cb(target, event)
# end

# event_cb = tasmota.gen_cb(event_cb_dispatch)
...
# prev_btn.add_event_cb(btn_clicked_cb, lv.EVENT_ALL, 0)
