...
bool parseDeviceInnerData(class Z_Device & device, JsonParserObject root) {
  for (auto data_elt : root) {
    ...
    if ((data_type == Z_Data_Type::Z_Unknown) || (data_type_str[1] != '-')) {
    ...
    uint8_t endpoint = strtoul(&data_type_str[2], nullptr, 16);   // hex base 16
    JsonParserToken val;
...
void CmndZbData(void) {
  ...
  if (XdrvMailbox.data[0] == '{') {
    ...
    {   // scope to force object deallocation
      ...
      attr_data.addAttribute(F("_-00")).setStrRaw(device_attr.toString(true).c_str());
    }
