...
typedef union {                            // Restricted by MISRA-C Rule 18.4 but so useful...
  ...
  struct {                                 // SetOption82 .. SetOption113
    ...
    uint32_t compress_rules : 1;           // bit 11 (v8.2.0.6)  - SetOption93 - Enables compression for Rules, should allow to double the size of rules
    uint32_t compress_rules_cpu : 1;       // bit 12 (v8.2.0.6)  - SetOption94 - Keep uncompressed rules in memory to avoid CPU load of uncompressing at each tick
    uint32_t spare13 : 1;
    uint32_t spare14 : 1;
    uint32_t spare15 : 1;
    uint32_t spare16 : 1;
