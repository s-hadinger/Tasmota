...
typedef union {                            // Restricted by MISRA-C Rule 18.4 but so useful...
  ...
  struct {                                 // SetOption82 .. SetOption113
    ...
    uint32_t mqtt_tls : 1;                 // bit 20 (v8.4.0.1)  - SetOption102 - Enable TLS mode (requires TLS version)
    uint32_t mqtt_no_retain : 1;           // bit 21 (v8.4.0.1)  - SetOption103 - No Retain - disable all MQTT retained messages, some brokers don't support it: AWS IoT, Losant
    uint32_t spare22 : 1;
    uint32_t spare23 : 1;
    uint32_t spare24 : 1;
    uint32_t spare25 : 1;
